...
const parseEndDate = (end: string) => {
    const currentMonthYear = `${new Date().getMonth() + 1}/${new Date().getFullYear()}`;
    return end.includes(currentMonthYear) ? 'Present' : end;
};

const TimelineCard: React.FC<TimelineCardProps> = ({ company, end, start, description, position, logo, styling }) => {
    return (
        <div>
